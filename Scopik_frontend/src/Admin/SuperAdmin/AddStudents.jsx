...
function AddStudent() {
  ...
  const fetchUniversities = () => {
    axios
      ...
      .catch((err) => console.log("Error fetching universities:", err));
  };

  const filteredStudents = student.filter((s) => {
    const matchName = s.name.toLowerCase().includes(search.toLowerCase());
    const matchYear = filterYear ? String(s.academicYear) === filterYear : true;

    if (activeTab === "all") return matchName && matchYear;
    ...
    return (
      <div className="flex flex-col gap-3">
        ...
        {filteredStudents.map((stu, index) => (
          <div
            key={index}
            className="flex items-center gap-3 border-b py-2 px-2 bg-gray-50 rounded hover:bg-gray-100 transition"
          >
            <input
              type="checkbox"
            ...
            >
              ...
              <span className="text-sm text-gray-600">{stu.email}</span>
    ...
    <div className="w-full rounded-xl min-h-screen bg-gray-50 px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row justify-end gap-3 mb-4">
        <div className="flex gap-2">
          {["ALL", "University", "Common Students"].map((tab) => (
            <button
              key={tab}
        ...
        <select
          ...
          className="p-2 rounded border text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
          ...
          {uploadType === "bulk" && (
            <div className="flex flex-col gap-3">
              ...
              <input
                type="file"
                onChange={bulkFile}
                className="border border-dashed border-blue-400 p-5 rounded cursor-pointer bg-white hover:bg-blue-50 text-sm text-gray-700 transition"
              />
              <button
      ...
      {studentModalOpen && selectedStudent && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="relative bg-white p-6 w-full max-w-6xl rounded-lg shadow-lg max-h-[90vh] overflow-y-auto">
            <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div>
                <label className="text-black font-semibold">Register No</label>
                <p className="text-gray-800">
            ...
            <hr className="my-4" />
            <h3 className="text-xl font-semibold text-black mb-3">Payments</h3>
