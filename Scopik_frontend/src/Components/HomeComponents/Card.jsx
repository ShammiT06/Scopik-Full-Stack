...
export default function NewsSlider() {
 ...
 className={`news__item p-8 z-20 rounded-xl transition-all duration-500 h-[600px] flex flex-col justify-between border group ${
  isActive
    ? "text-black bg-white shadow-2xl border-transparent"
    : `${isDarkMode ? "text-white" : "text-black"} bg-white/10 border-white/30 backdrop-blur-md hover:bg-white/20 hover:backdrop-blur-lg`
} ${
  isActive || isNext || isPrev
...
>
          ...
          <div>
            ...
            <p className="text-base md:text-lg opacity-70 leading-relaxed font-news transition-colors">
          ...
          <Link to={`/course/${item.id}`}>
           <button
  className={`w-full mt-6 py-3 px-6 border border-white/20 font-semibold text-lg rounded-xl shadow-md transition duration-300 ease-in-out ${
    isActive
      ? "bg-[#F97316] text-black shadow-xl scale-105"
      : "bg-white/10 text-white group-hover:bg-[#FF8011] group-hover:text-black"
  }`}
>
  View Course
</button>
